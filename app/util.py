...
def validate_token_name(token_name):
    if not isinstance(token_name, six.string_types):
        return True

    if len(token_name) > TOKEN_NAME_MAX_LENGTH:
        return True

    if not token_name.replace(' ', '').isalnum():
        return True
